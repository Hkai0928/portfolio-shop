var app = new Vue({
  ...
  created: function(){
    ...
    if(typeof(document.getElementById('address').value) != 'undefined') {
  ...
  computed: {
    ...
    isActive() {
      ...
      if(url[url.length-1] == "mem_edit.php" && this.password == "") {
        this.activeCount = 3;
      } else if(url[url.length-1] == "user_edit.php" && this.password == "") {
        this.activeCount = 3;
      } else {
        this.activeCount = 5;
      }
  ...
  watch: {
    ...
    address(address) {
      if(address != "") {
        this.$set(this.inputCounts, "address")
      } else {
        this.$delete(this.inputCounts, 'address')
      }
    },
    loginId(loginId) {
